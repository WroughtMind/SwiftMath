...
class MTFontMathTable {
   ...
   var stretchStackTopShiftUp:CGFloat { constantFromTable("StretchStackTopShiftUp") }
    ...
    var displayOperatorMinHeight:CGFloat { constantFromTable("DisplayOperatorMinHeight") }
    ...
    func getLargerGlyph(_ glyph:CGGlyph, forDisplayStyle: Bool = false) -> CGGlyph {
        ...
        if forDisplayStyle {
            var largest = glyph
            for variant in getVerticalVariantsForGlyph(glyph).compactMap({ $0 }) {
                ...
                if bounds.height >= displayOperatorMinHeight { return candidate }
            }
            return largest
        } else {
